const SPICE_TWENTY_FOUR = {
    id: 7,
    name: 'Spice 24',
    dishes: [
         { id: 72, name: "Create Your Own Stir-Fry", price:"SP",
                options: {
                    optionSize:
                        {
                        name: "Choose Favors ",
                        choices: [
                            "麻辣香鍋 $1.75",
                            "亞洲香鍋 $1.00",
                            "豆豉香鍋 $1.00",
                            "薑蔥香鍋 $1.00",
                            "黑胡椒香鍋 $1.00",
                            "麻辣燙 $1.75",
                            "牛奶麻辣燙 $1.75",
                            "牛奶麻辣燙 $1.75",
                            "蕃茄湯 $1.00",
                            "藤椒冒菜 $1.00",
                        ], selectedOptions: [], limit: 1, adjustable: true
                    },
                    option2:
                        {
                         name: "Choose Spiciness",
                         choices: [ "Mild 微辣", "Medium 中辣", "Hot大辣", "Extra hot 特辣"], selectedOptions: [], limit: 1
                    },
                    option3: {
                        name: "Ingredients (4.95 each)",
                        choices: [
                            "肥牛($4.95)",
                            "牛排($4.95)",
                            "肥羊 ($4.95)",
                            "五花肉($4.95)",
                            "雞肉 ($4.95)",
                            "鴨血($4.95)",
                            "牛柏葉($4.95)",
                            "肥腸($4.95)",
                            "魚片($4.95)",
                            "帶子($4.95)",
                            "魷魚($4.95)",
                            "蝦仁($4.95)",
                            "魚豆腐($4.95)",
                            "蟹肉棒($4.95)",
                            "貢丸($4.95)",
                            "魚蛋($4.95)",
                            "午餐肉($4.95)",
                            "親親腸($4.95)",
                            "鵪鶉蛋($4.95)",
                        ],
                        selectedOptions: [],
                        limit: 100,
                        price: 4.95
                    },
                    option4: {
                        name: "Ingredients (3.95 each)",
                        choices: [
                            "千葉豆腐($3.95)",
                            "腐竹($3.95)",
                            "豆皮($3.95)",
                            "油豆腐($3.95)",
                            "土豆($3.95)",
                            "藕片($3.95)",
                            "芋絲札($3.95)",
                            "海帶絲($3.95)",
                            "黑木耳($3.95)",
                            "白玉菇($3.95)",
                            "玉子菇($3.95)",
                            "金針菇($3.95)",
                            "菠蘿 ($3.95)",
                            "大白菜($3.95)",
                            "西蘭花($3.95)",
                            "花菜($3.95)",
                            "四季豆($3.95)",
                            "秋葵($3.95)",
                            "蘆筍($4.95)",
                            "筍尖($3.95)",
                            "酸菜($3.95)",
                            "包菜($3.95)",
                            "黃豆芽($3.95)",
                            "上海菜($3.95)",
                            "油麥菜($3.95)",
                            "萵筍($3.95)",
                            "玉米($3.95)",
                            "甜豆($3.95)",
                            "年糕($3.95)",
                            "寬粉($3.95)",
                            "蛋麵($3.95)",
                            "紅薯粉($3.95)",
                            "烏冬麵($3.95)",
                            "公仔麵($3.95)",
                            "兩面黃($3.95)"
                        ],
                        selectedOptions: [],
                        limit: 100,
                        price: 3.95
                    },
                    option5: {
                        name: "Toppings ", choices: ["青蔥", "香菜", "芝麻", "油蔥酥 $1.50", "黃金炒蛋 $3.50"], selectedOptions: [], limit: 100, adjustable: true
                    }
        }},
        { id: 759, name: '冒烤鴨 Roast Duck In Chill Oil ', price:58.95},
        { id: 758, name: '滷味拼盤(滷豆腐，滷蛋，滷鍵 Colf Cut Platter includes Braised Tofu,Egg and Beef)', price:9.95 },
        { id: 73, name: '燒椒茄子 Spicy Eggplant', price:9.95},
        { id: 74, name: '雞翅 Chicken Wings ', price:13.95 , options: {option1: {name: "Types", choices: ["黑胡椒 Black Peppers", "麻辣 Mala", ], selectedOptions: [], limit: 1}}},
        { id: 75, name: '撈汁秋葵 Okra Salad', price:9.95},
        { id: 76, name: '夫妻肺片 Beef and Tendon in Chili Oil', price:12.95 },
        { id: 77, name: '黑椒牛肉餡餅 Pepper Beef Empanadas', price:7.95},
        { id: 79, name: '鹽酥雞 Popcorn Chicken', price:8.95 },
        { id: 710, name: '蔥油餅 Scallian Pancake', price:6.95},
        { id: 711, name: '椒鹽乾炸肥腸 Cripsy Intestine', price:10.95},
        { id: 712, name: '南乳脆五花 Cripsy Pork Belly', price:9.95},
        { id: 713, name: '美極醬黃瓜 Picked Cucumber ', price:7.95},
        { id: 714, name: '古法酥炸丸子 Cripsy Meatball', price:9.95},
        { id: 715, name: '紅油乾炒拌手 Pork Woton in Chill oil', price:8.95},
        { id: 756, name: '拔絲香芋 Pulled Sugar Taro', price:13.95 },
        { id: 757, name: '韭黃海鮮卷 Seafood Spring Roll (4pc)', price:9.95 },
        { id: 716, name: '紅糖糍粑 Mochi with Brown Sugar', price:7.95},
        { id: 714, name: '港式京都骨 Pork Chop in Peaking Sauce', price:19.95},
        { id: 718, name: '香椿炒雞蛋', price:17.95  },
        { id: 719, name: '菠蘿辣子雞', price:18.95 },
        { id: 720, name: '雞卜卜脆炒飯', price:22.95  },
        { id: 721, name: '金鑽孜然排骨', price:19.95  },
        { id: 722, name: '秘製鹽焗雞半隻', price:22.95 },
        { id: 757, name: '辣炒花甲 spicy Claims', price:23.95 },
        { id: 723, name: '豆腐燒黃魚', price:28.95  },
        { id: 724, name: '板栗燒雞', price:25.95  },
        { id: 725, name: '火爆腰花', price:17.95  },
        { id: 726, name: '砂鍋臘肉花菜', price:18.95 },
        { id: 727, name: '上湯豆苗', price:23.95 },
        { id: 728, name: '手撕包菜', price:13.95 },
        { id: 729, name: '避風塘軟殼蟹', price:28.95 },
        { id: 730, name: '仙露筍牛肉粒', price:28.95 },
        { id: 731, name: '梅菜排骨燜肉 Braised Pork& ribs with/ Mustard Greens', price:25.95 },
        { id: 732, name: '湘西土匪豬肝', price:19.95},
        { id: 733, name: 'XO醬小炒皇', price:28.95 },
        { id: 734, name: '石鍋涼粉', price:10.95 },
        { id: 735, name: '豆角燒茄子', price:16.95 },
        { id: 736, name: '麻婆豆腐牛腩', price:19.95 },
        { id: 737, name: '金沙南瓜蝦球', price:26.95 },
        { id: 738, name: '燒汁牛小排', price:29.95 },
        { id: 739, name: '秘製鮮魷魚', price:16.95},
        { id: 740, name: '老馬蹄花湯', price:18.95},
        { id: 741, name: '酸辣土豆絲', price:13.95 },
        { id: 742, name: '乾煸四季豆', price:13.95 },
        { id: 743, name: '擔擔麵', price:8.95 },
        { id: 744, name: '滑蛋濕炒河粉', price:18.95},
        { id: 745, name: '素菜兩面黃', price:13.95 },
        { id: 746, name: '黑松露炒飯', price:16.95 },
        { id: 747, name: '港式臘味炒飯', price:18.95 },
        { id: 748, name: '成都辣炒飯 (spicy) 🌶️', price:15.95 },
        { id: 749, name: '菠蘿蝦仁炒飯', price:13.95},
        { id: 750, name: '紅燒牛腩麵 (spicy）🌶️', price:12.95},
        { id: 751, name: '清湯素菜麵 ', price:12.95},
        { id: 752, name: '炒手', price:10.95 , options: {option1: {name: "Types", choices: ["老麻", "骨湯", ], selectedOptions: [], limit: 1 }}},
        { id: 753, name: '涼麵', price:8.95 , options: {option1: {name: "Types", choices: ["麻辣", "芝麻", ], selectedOptions: [], limit: 1 }}},
        { id: 754, name: '兩面黃', price:15.95 , options: {option1: {name: "Types", choices: ["雞肉", "蝦仁", "牛肉", ], selectedOptions: [], limit: 1}}},
        { id: 755, name: '炒麵', price:13.95 , options: {option1: {name: "Types", choices: ["雞肉", "蝦仁", "牛肉", ], selectedOptions: [], limit: 1 }}},
    ]
};

export default SPICE_TWENTY_FOUR;
