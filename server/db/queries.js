...
const createUser = async (username, passwordHash, phoneNumber, verificationCode, verificationExpiresAt) => {
  const result = await pool.query(
    "INSERT INTO users (username, password_hash, phone_number, verification_code, verification_expires_at) VALUES ($1, $2, $3, $4, $5) RETURNING id, username, phone_number, is_verified, created_at, role",
...
const getUserByUsernameAndPhone = async (username, phoneNumber) => {
  const result = await pool.query("SELECT * FROM users WHERE username = $1 AND phone_number = $2", [
    username,
    phoneNumber,
  ]);
  return result.rows[0];
